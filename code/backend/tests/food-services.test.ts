...
import { food_results, food_facts_egg, egg, food_facts_egg_2, food_facts_egg_3, egg_2, food_facts_egg_4 } from "./files/food.ts";
...
describe("searchFood", () => {
  ...
  it("throws exception if there's no matching elements", async () => {
    await expect(services.searchFood("notAFood", 0, 0)).rejects.toThrow("NotFoundError");
  });
