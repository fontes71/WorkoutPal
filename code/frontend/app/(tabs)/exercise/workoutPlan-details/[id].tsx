...
import {
  Exercise,
  ExercisesFromWorkoutPlanResponse,
  WorkoutPlan,
  WorkoutPlanResponse,
...
const TopSection: React.FC<TopSectionProps> = ({
  ...
  const onSaveHook = async (workoutPlan: WorkoutPlan) => {
    ...
    const response = await fetch(
      `${localhost}8080/api/exercises/workoutPlans/log`,
      {
        method: "POST",
        headers: {
...
const WorkoutPlanDetailsScreen = () => {
  ...
  useEffect(() => {
    const fetchExercises = async () => {
      try {
        ...
        const response = await fetch(
          `${localhost}8080/api/exercises/workoutPlans/${workoutPlan.name}`,
          {
            method: "GET",
            headers: {
  ...
  const handleDeletePress = async (
    ...
    const response = await fetch(
      `${localhost}8080/api/exercises/workoutPLans/${workoutPlanName}/exercise/${exerciseId}`,
      {
        method: "DELETE",
        headers: {
